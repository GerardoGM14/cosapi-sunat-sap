...
export class SapSunatComponent implements OnInit {
  ...
  loadSapConfig() {
    this.http.get<any>(`${this.configService.apiUrl}/utils/config/sap`)
      .subscribe({
        next: (res) => {
          if (res.usuario) {
            this.configData.sap.usuario = res.usuario;
          }
          if (res.password) {
            this.configData.sap.password = res.password;
          }
        },
        error: (err) => {
          console.error('Error loading SAP config', err);
        }
      });
  }
